...
public class RequestHandler {
    ...
    public void handleQueryParameters(JsonNode jsonObject, ZagMethod method) {
        ...
        if (queryParams != null && !queryParams.isEmpty()) {
            queryParams.fieldNames().forEachRemaining(query -> method.getData().queryParams.put(query, queryParams.get(query).asText()));
        }
    }
    ...
    public void handleHeaders(JsonNode jsonObject, ZagMethod method) {
        ...
        if (headers != null && !headers.isEmpty()) {
            headers.fieldNames().forEachRemaining(header -> method.getData().headers.put(header, headers.get(header).asText()));
        }
    }
        ...
        if (path != null && path.contains("?")) {
            ...
            if (!queryString.isEmpty()) {
                ...
                for (String queryParam : queryParams) {
                    ...
                    if (!method.getData().queryParams.containsKey(key)) {
                        method.getData().queryParams.put(key,value);
                    }
                }
            }
        }
